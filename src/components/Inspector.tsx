...
const Inspector = ({
  ...
  /* New Data Extraction */
  const m = metric;
  const question = m.question; // question is optional in Metric, might need to handle logic if coreQuestion exists? Metric type doesn't have coreQuestion.
  // Original code had: const question = m.coreQuestion || m.question;
  // If Metric type only has question, then just m.question.

  // Extract criteria from new structure or fallback
  let criteriaLow = { label: "Low Confidence", description: "Information is abstract, lacks referential stability, and cannot be acted upon." };
  let criteriaHigh = { label: "High Confidence", description: "Concrete details provided. Identifies specific actors, programs, or mechanisms." };

  if (m.scoringCriteria && m.scoringCriteria.length >= 5) {
    const low = m.scoringCriteria.find((c) => c.score === 1);
    const high = m.scoringCriteria.find((c) => c.score === 5);
    if (low) criteriaLow = { label: low.label, description: low.description };
    if (high) criteriaHigh = { label: high.label, description: high.description };
  } else if (m.criteria) {
    // Legacy Fallback
    criteriaLow.label = m.criteria.split(", ")[0].split("=")[1] || "Low Confidence";
    criteriaHigh.label = m.criteria.split(", ")[1]?.split("=")[1] || "High Confidence";
  }

  // Handle missing community score
