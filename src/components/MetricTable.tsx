...
interface MetricTableProps {
    ...
    onVoteChange?: (id: number | string, field: "confidence" | "notes", value: string | null) => void | Promise<void>;
        ...
        const baseCols: ColumnDef<Metric>[] = [
            {
                ...
                meta: { className: "p-4 w-12 text-center text-ops-text-dim font-mono" },
            },
            {
                header: "Metric Name",
                accessorKey: "name",
            ...
            {
                header:
                    mode === "governance" ? "Current Definition (Standard)" : "Assessment Criteria",
                accessorKey: "question",
                cell: ({ row, table }) => {
                    const metric = row.original;
                    // Properly cast meta to access custom props
                    const meta = table.options.meta as any;
                    const { mode, scores, onScoreChange } = meta;

                    // Extract min/max labels/descriptions from scoringCriteria (now on metric object from DB)
                    ...
                    if (lowCriteria) {
                        ...
                        minDesc = metric.lowDescription; // For 11-20, we put text in description
                        minLabel = "Low"; // Simplified label
                    }
                        ...
                        return (
                            <div className="text-xs text-gray-400">
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-2 opacity-60">
                                    <span className="text-red-400">1: {minLabel}{minDesc ? `: ${minDesc}` : ''}</span>
                                    <span className="text-ops-accent">5: {maxLabel}{maxDesc ? `: ${maxDesc}` : ''}</span>
                                </div>
                            </div>
                        );
                    }
                            ...
                            {mode === "scoring" ? (
                                < div className="flex items-center gap-4">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            onScoreChange?.(metric.id, parseInt(e.target.value))
                                        }
        ...
        if (mode === "governance") {
            baseCols.push({
                ...
                cell: ({ row, table }) => {
                    const metric = row.original;
                    const meta = table.options.meta as any;
                    const { votes, onVoteChange, saveStatus } = meta;
                    const vote = votes ? votes[metric.id] : {};
                            ...
                            <div className="flex gap-1">
                                {confidenceLevels.map((level) => (
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            onVoteChange?.(metric.id, "confidence", level.value);
                                        }}
                                        className={`
            ...
            // Extracted Component for Metric Note Cell
            ...
            baseCols.push({
                ...
                cell: ({ row, table }) => {
                    const metric = row.original;
                    const meta = table.options.meta as any;
                    const { votes, onVoteChange } = meta;
                    const vote = votes ? votes[metric.id] : {};

                    return (
                        <MetricNoteCell
                            notes={vote?.notes}
                            onUpdate={(val) => onVoteChange?.(metric.id, "notes", val)}
                        />
                    );
                },
                meta: { className: "p-4" },
                            ...
                            {mode === "review" ? (
                                <div className="flex flex-col items-center">
                                    <span className="text-sm mono font-bold text-white">
                                        {metric.communityScore ? metric.communityScore.toFixed(1) : "-"}
                                    </span>
                                    <div className="w-16 h-1 bg-border mt-1 rounded-full overflow-hidden">
                                        <div
                                            className="h-full bg-primary"
                                            style={{ width: `${(metric.communityScore || 0 / 5) * 100}%` }}
                                        ></div>
                                    </div>
                                </div>
                            ) : (
                                <span
                                    className={`text-lg mono font-bold ${score && score > 0 ? "text-primary" : "text-text-muted"}`}
                                >
                                    {score && score > 0 ? score : "-"}
                                </span>
                            )}
