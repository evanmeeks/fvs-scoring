...
export default function UserManagementPanel({
  ...
  const checkAdminAndLoadUsers = async () => {
    ...
    try {
      const { data: isAdminData, error: roleError } =
      ...
      const { data: profilesData, error: profilesError } = await supabase.rpc(
        "get_admin_user_list" as any,
      );

      if (profilesError) {
        console.error("Error loading profiles:", profilesError);
        if (profilesError.code === "PGRST202") {
          setError(
            "System Update Required: Missing admin functions. Please apply latest database migrations.",
          );
        } else {
  ...
  const handlePromoteToAdmin = async (userId: string) => {
    ...
    try {
      const { data, error } = await (supabase.rpc as any)("promote_user_to_admin", {
      ...
      setSuccessMessage(
        (data as any)?.message || "User promoted to admin",
      );
  ...
  const handlePromoteToContributor = async (userId: string) => {
    if (
      !confirm("Are you sure you want to promote this user to contributor?")
    )
      return;
    ...
    try {
      ...
      setSuccessMessage(
        (data as any)?.message || "User promoted to contributor",
      );
      setTimeout(() => setSuccessMessage(null), 3000);
      await checkAdminAndLoadUsers();
    } catch (err: any) {
  ...
  const handleDemoteContributorToUser = async (userId: string) => {
    if (
      !confirm(
        "Are you sure you want to demote this contributor to regular user?",
      )
    )
      return;
    ...
    try {
      ...
      setSuccessMessage(
        (data as any)?.message || "Contributor demoted to user",
      );
      setTimeout(() => setSuccessMessage(null), 3000);
      await checkAdminAndLoadUsers();
    } catch (err: any) {
  ...
  const handleDemoteToUser = async (userId: string) => {
    if (
      !confirm("Are you sure you want to demote this admin to regular user?")
    )
      return;

    try {
      const { data, error } = await (supabase.rpc as any)("demote_admin_to_user", {
        target_user_id: userId,
      });
      ...
      setSuccessMessage(
        (data as any)?.message || "Admin demoted to user",
      );
      setTimeout(() => setSuccessMessage(null), 3000);
      await checkAdminAndLoadUsers();
    } catch (err: any) {
                    ...
                    <div className="flex items-center">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {profile.is_verified && (
                            <span
                              className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-blue-900/50 text-blue-400"
                              title="Verified User"
                            >
                              ✓
                            </span>
                        ...
                        <div
                          className="text-[10px] text-text-muted/40 font-mono mt-0.5"
                        ...
                        <div className="mt-1 flex flex-col gap-0.5 text-xs text-text-muted/60">
                          ...
                          {profile.bio && (
                            <div
                              className="italic mt-0.5 text-text-muted/50 truncate max-w-xs"
                              title={profile.bio}
                            >
                              {profile.bio}
                            </div>
                      ...
                      <>
                        {profile.role === "user" && (
                          <div className="flex items-center justify-end gap-3">
                            <button
                              onClick={() =>
                                handlePromoteToContributor(profile.user_id)
                              }
                            ...
                            <button
                              onClick={() =>
                                handlePromoteToAdmin(profile.user_id)
                              }
                        ...
                        {profile.role === "contributor" && (
                          <div className="flex items-center justify-end gap-3">
                            <button
                              onClick={() =>
                                handleDemoteContributorToUser(profile.user_id)
                              }
                        ...
                        {profile.role === "admin" && (
                          <button
                            onClick={() =>
                              handleDemoteToUser(profile.user_id)
                            }
                            className="text-orange-500 hover:text-white font-medium"
                          >
                            Demote to User
