...
import { useUI, useMetrics } from "~/context/AppContext";
...
export function SubmissionModal() {
  ...
  const [mode, setMode] = useState<"rfc" | "new-metric">("rfc");
  ...
  const getMetricCriteria = (metric: any) => {
    ...
    if (metric.scoringCriteria && Array.isArray(metric.scoringCriteria)) {
      const minScore = metric.scoringCriteria.find(
        (c: any) => c.score === 1,
      );
      const maxScore = metric.scoringCriteria.find(
        (c: any) => c.score === 5,
      );
      return {
        minCriteria: minScore
          ? `${minScore.label}: ${minScore.description}`
          : metric.lowDescription || "",
        maxCriteria: maxScore
          ? `${maxScore.label}: ${maxScore.description}`
          : metric.highDescription || "",
      };
    }
    return {
      minCriteria: metric.lowDescription || "",
      maxCriteria: metric.highDescription || "",
    };
  };
  ...
  useEffect(() => {
    if (submissionModalOpen) {
      const initialId = selectedMetric?.id
        ? String(selectedMetric.id)
        : FVS_METRICS[0]
          ? String(FVS_METRICS[0].id)
          : "1";
      const metric = FVS_METRICS.find((m) => String(m.id) === initialId);
      const criteria = getMetricCriteria(metric);

      setRfcForm({
        targetMetric: initialId,
        contributionType: "RFC (Request for Comment)",
        rationale: "",
  ...
  }, [submissionModalOpen, selectedMetric, FVS_METRICS]);

  if (!submissionModalOpen) return null;

  const onClose = () => setSubmissionModalOpen(false);
  ...
  const handleSubmit = async () => {
    ...
    const insertData: any = {
      proposal_type: mode === "rfc" ? "rfc" : "new_metric",
      rationale: payload.rationale,
      user_id: user.id,
      proposed_name: payload.metricName || null,
      proposed_question: payload.metricQuestion || null,
      proposed_min_criteria: payload.minCriteria || null,
      proposed_max_criteria: payload.maxCriteria || null,
      status: "pending",
    };

    if (mode === "rfc" && rfcForm.targetMetric) {
      insertData.metric_id = Number(rfcForm.targetMetric);
    }

    if (payload.richEntries) {
      insertData.rich_entries = payload.richEntries;
    }

    const { error } = await (supabase.from("rfc_proposals") as any).insert(
      insertData,
    );
      ...
      <div className="bg-panel w-full max-w-2xl border border-border rounded-lg shadow-2xl flex flex-col max-h-[90vh] scale-100 animate-slide-up">
        ...
        <div className="p-4 border-b border-border bg-background">
          <div className="flex gap-2">
            <button
              onClick={() => setMode("rfc")}
              className={`flex-1 py-2 px-4 rounded text-xs font-bold uppercase tracking-wide transition-all ${
                mode === "rfc"
            ...
            <>
              ...
              <div className="bg-background border border-border rounded-lg p-4">
                ...
                <p className="text-xs text-text-muted mb-3">
                  Select an existing metric to instantly copy its structure. The
                  form will auto-populate and you can modify the fields below.
                </p>
