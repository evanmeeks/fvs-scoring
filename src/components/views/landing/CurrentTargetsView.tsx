...
export const CurrentTargetsView = () => {
    ...
    const handleTargetClick = (target: TargetWithScore) => {
        // Navigate to Consensus tab with target slug
        const slug = generateTargetSlug(target);
        navigate({ to: '/global-consensus/$slug', params: { slug } });
    };
    ...
    const getStatusIcon = (statusLabel: string) => {
        if (statusLabel === 'AUDITED') return 'check_circle';
        if (statusLabel.includes('OCCUPATION')) return 'block';
        return 'pending';
    };

    return (
        <div id="view-targets" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-ops-text-dim text-sm mt-1">
                        Select a target to view consensus data, execute audit, or explore intelligence.
                    </p>
                </div>
            ...
            {loading && (
                <div className="text-center py-12">
                    ...
                    <p className="text-ops-text-dim text-sm mt-4 font-mono">Loading targets...</p>
                </div>
            )}
            ...
            {!loading && !error && filteredTargets.length === 0 && searchQuery && (
                <div className="text-center py-12">
                    <p className="text-ops-text-dim text-sm font-mono">No targets found matching "{searchQuery}"</p>
                </div>
            )}

            {!loading && !error && filteredTargets.length === 0 && !searchQuery && (
                <div className="text-center py-12">
                    <p className="text-ops-text-dim text-sm font-mono">No targets available</p>
                </div>
            )}

            <div id="targets-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredTargets.map((target) => (
                    <div
                        key={target.id}
                        role="button"
                        tabIndex={0}
                        className={`bg-ops-panel border ${target.borderClass} p-5 rounded-lg hover:bg-white/5 transition-all group flex flex-col justify-between h-full cursor-pointer`}
                    ...
                    >
                        <div>
                            ...
                            <div className="flex items-center gap-2 mb-4">
                                <span className={`material-symbols-outlined text-sm ${target.colorClass}`}>
                        ...
                        <div className="border-t border-ops-border pt-4 flex justify-between items-center mt-auto">
                            <div className={`text-2xl font-black ${target.colorClass}`}>
                                {target.scoreCount > 0 ? `${target.totalScore}/100` : '--/100'}
                            </div>
                            <button
                                className="bg-ops-border hover:bg-ops-accent hover:text-black text-white text-xs font-mono uppercase px-3 py-2 rounded transition-colors flex items-center gap-1"
                                onClick={(e) => {
                                    e.stopPropagation();
                                    handleTargetClick(target);
                                }}
