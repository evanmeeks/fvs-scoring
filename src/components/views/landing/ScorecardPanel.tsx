...
import { useMetrics, useTarget, useScoring } from "~/context/AppContext";
import {
  Radar,
  RadarChart,
  PolarGrid,
...
export const ScorecardPanel = React.memo(() => {
  const { FVS_METRICS } = useMetrics();
  const { activeTarget } = useTarget();
  const { scores, totalScore } = useScoring();
          ...
          <div className="bg-ops-panel/80 backdrop-blur border border-ops-border rounded-xl p-6 relative overflow-hidden group">
            ...
            {activeTarget && (
              <div className="grid grid-cols-3 gap-6 mt-5 pt-5 border-t border-ops-border/50">
                <div>
                  <div className="text-[10px] font-mono text-ops-text-dim uppercase tracking-wider mb-1">
                    Case ID
                  </div>
                ...
                <div>
                  ...
                  <div className="text-sm font-mono text-white">
                    {activeTarget.claim_date
                      ? new Date(
                          activeTarget.claim_date as string,
                        ).toLocaleDateString()
                      : "N/A"}
                  </div>
                </div>
              ...
              return (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-center w-12">
                        <span
                          className={`text-2xl font-black block ${colorClass}`}
                        >
                          {val || "-"}
                        </span>
                      </div>
                  ...
                  <p className="text-gray-400 text-sm mb-4 border-l-2 border-gray-700 pl-3 italic">
                    &quot;
                    {(metric as any).coreQuestion ||
                      (metric as any).question ||
                      metric.description}
                    &quot;
                  </p>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="lg:sticky lg:top-24 space-y-6">
            ...
            <div className="bg-ops-panel/80 backdrop-blur border border-ops-border rounded-xl p-6 relative overflow-hidden">
              ...
              <svg
                className="absolute right-4 top-4 w-20 h-20 text-ops-border opacity-30 pointer-events-none"
                viewBox="0 0 100 100"
              >
                <path
                  d="M10 50 H40 V20 H70"
                  stroke="currentColor"
                  fill="none"
                  strokeWidth="1"
                />
                <path
                  d="M30 70 H60 V40 H90"
                  stroke="currentColor"
                  fill="none"
                  strokeWidth="1"
                />
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-3">
                  <span
                    ...
                    style={{
                      textShadow:
                        normalizedScore > 0
                          ? `0 0 30px ${classification.chartColor}40, 0 0 60px ${classification.chartColor}20`
                          : "none",
                    }}
                  >
                    {normalizedScore}
                  </span>
                  <span className="text-2xl text-ops-text-dim font-mono">
                    / 100
                  </span>
                </div>
                ...
                <div className="w-full bg-ops-black h-2.5 rounded-full mt-6 overflow-hidden border border-ops-border">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: classification.chartColor,
                    }}
                  ></div>
                </div>
                ...
                <div className="mt-6 pt-5 border-t border-ops-border/50 flex justify-between items-center">
                  <div>
                    ...
                    <div
                      className={`text-xl font-bold transition-colors duration-500 ${classification.color} ${normalizedScore > 0 ? "animate-pulse" : ""}`}
                    >
                      {classification.label}
                    </div>
                  </div>
                  <div
                    className={`h-10 w-10 rounded flex items-center justify-center ${classification.iconBg}`}
                  >
                    <span
            ...
            <div className="bg-ops-panel/80 backdrop-blur border border-ops-border rounded-xl p-5 relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-ops-border/50 relative z-10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-mono text-ops-text-dim">
                    VERIFIABILITY
                  </span>
                </div>
                <div className="flex items-center gap-2">
