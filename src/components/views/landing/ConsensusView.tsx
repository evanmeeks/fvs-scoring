...
import { useTargetsWithScores, type TargetWithScore } from "../../../hooks/useTargetsWithScores";
import {
  BarChart,
  Bar,
  XAxis,
...
import { FVS_METRICS, type QddMetric } from "../../../data/metrics.ts";
...
export const ConsensusView = ({ activeTargetSlug }: ConsensusViewProps) => {
  ...
  // Fetch metric-level scores for the active target
  useEffect(() => {
    async function fetchMetricScores() {
      ...
      if (fetchError) {
        console.error("Failed to fetch metric scores:", fetchError);
        return;
      }

      // Group by metric_id and calculate averages
      const metricMap: Record<number, { total: number; count: number }> = {};
      type VoteRow = { metric_id: number | null; vote_value: number | null };
      (data as VoteRow[] | null)?.forEach((vote) => {
        if (vote.metric_id == null || vote.vote_value == null) {
          return;
        }
      ...
      // Convert to MetricScore array with metric names
  ...
  const globalStats = useMemo(() => {
    ...
    const totalScores = scoredTargets.reduce((sum, t) => sum + t.totalScore, 0);
    const avgScore =
  ...
  // Prepare data for top targets bar chart
  ...
  // Prepare data for classification distribution (radar chart)
  const classificationRadarData = useMemo(() => {
    ...
    return classifications.map((classification) => {
      const count =
        (globalStats.classificationDistribution as Record<string, number>)[
        classification
        ] || 0;
      return {
  ...
  const handleTargetClick = (target: TargetWithScore) => {
    // Navigate to Audit Target tab with target slug
    const slug = generateTargetSlug(target);
    navigate({ to: "/audit-target/$slug", params: { slug } });
  };
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
        ...
        <div className="bg-ops-panel border border-ops-border p-6 rounded-lg relative overflow-hidden hover:border-ops-accent/50 transition-all group">
          ...
          <div
            className={`text-4xl font-black ${globalStats.avgScore >= 76
              ? "text-cyan-400"
      ...
      {/* Metric-Level Breakdown for Active Target */}
      {
            ...
            <ResponsiveContainer width="100%" height={400}>
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value: number | undefined, _name: string | undefined, item: { payload?: MetricScore }) => [
                    `${(value ?? 0).toFixed(2)}/5.0 (${item?.payload?.voteCount ?? 0} votes)`,
                    "Average Score",
                  ]}
                />
                <Bar
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
            ...
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value: number | undefined, _name: string | undefined, item: { payload?: TopTargetDatum }) => [
                    `${value ?? 0}/100`,
                ...
                <Bar
                  ...
                  radius={[0, 4, 4, 0]}
                  onClick={(data) => {
                    const targetData = (data as { target?: TargetWithScore }).target;
                    if (targetData) {
        ...
        {/* Classification Distribution - Radar Chart */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {targets?.slice(0, 6).map((target) => (
            <button
              key={target.id}
              type="button"
              className={`w-full text-left bg-ops-panel border ${target.borderClass} p-4 rounded-lg hover:bg-white/5 transition-all cursor-pointer group`}
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <span className={`text-[10px] font-mono ${target.colorClass}`}>
