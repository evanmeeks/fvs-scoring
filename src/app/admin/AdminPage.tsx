          ...
          <div className="mb-6 flex justify-between items-end">
            <div>
              ...
              <p className="text-text-muted">
                Manage target submissions and system settings
              </p>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto w-full">
            ...
            {activeTab === "reviews" && (
              <div className="animate-fade-in">
                <ReviewQueueTable
                  onOpenFullPanel={() => setIsReviewOpen(true)}
                />
              </div>
            )}
          </div>
...
function TabButton({ active, onClick, icon, label }: TabButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`px-4 py-3 flex items-center gap-2 text-sm font-bold border-b-2 transition-all ${
        active
...
function StatCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={!onClick}
      className={`bg-panel border border-border ${border} border-l-4 rounded-lg p-5 text-left transition-all group ${
  ...
  useEffect(() => {
    const loadPendingSubmissions = async () => {
      try {
        ...
        const targetSubmissions: PendingSubmission[] = (targets || []).map(
          (t: any) => ({
            id: t.id,
        ...
        const rfcSubmissions: PendingSubmission[] = (rfcs || []).map(
          (r: any) => ({
            id: r.id,
            type: "rfc" as const,
    ...
    <div className="bg-panel border border-border rounded-lg overflow-hidden">
      <div className="px-6 py-4 border-b border-border bg-black/50 flex justify-between items-center">
        <h3 className="text-sm font-mono uppercase text-white flex items-center gap-2">
          <Icon name="warning" className="text-sm text-yellow-500" />
          Pending Submissions
        </h3>
        <button
