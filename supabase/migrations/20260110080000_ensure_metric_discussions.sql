-- Re-running creation of metric_discussions table to ensure existence
-- Original migration: 20260109160000_create_discussions.sql
...
CREATE INDEX IF NOT EXISTS idx_metric_discussions_metric_id ON metric_discussions(metric_id);
CREATE INDEX IF NOT EXISTS idx_metric_discussions_parent_id ON metric_discussions(parent_id);
CREATE INDEX IF NOT EXISTS idx_metric_discussions_created_at ON metric_discussions(created_at DESC);
...
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trigger_update_metric_discussions_updated_at ON metric_discussions;
...
DROP POLICY IF EXISTS "Anyone can view discussions" ON metric_discussions;
CREATE POLICY "Anyone can view discussions"
    ON metric_discussions
    FOR SELECT
    USING (true);
-- Policy: Authenticated users can insert discussions
DROP POLICY IF EXISTS "Authenticated users can insert discussions" ON metric_discussions;
CREATE POLICY "Authenticated users can insert discussions"
    ON metric_discussions
    FOR INSERT
    WITH CHECK (auth.uid() = user_id);
-- Policy: Users can update their own discussions
DROP POLICY IF EXISTS "Users can update their own discussions" ON metric_discussions;
CREATE POLICY "Users can update their own discussions"
    ON metric_discussions
    FOR UPDATE
    USING (auth.uid() = user_id);
-- Policy: Users can delete their own discussions
DROP POLICY IF EXISTS "Users can delete their own discussions" ON metric_discussions;
...
DROP POLICY IF EXISTS "Authenticated users can insert votes" ON discussion_votes;
...
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trigger_update_discussion_votes ON discussion_votes;
